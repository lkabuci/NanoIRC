...
Token    Lexer::get_next_token() {
    ...
    while (!_is_at_end()) {
        ...
        switch (c = _advance()) {
            case '\0':  break;
            case '&':   return Token(TYPES::AMPERSAND, "&");
            case '#':   return Token(TYPES::HASH, "#");
            case ',':   return Token(TYPES::COMMA, ",");
            case '+':   return Token(TYPES::PLUS, "+");
            case '-':   return Token(TYPES::MINUS, "-");
            case '\r':
                if (_match('\n'))
                    return Token(TYPES::CRLF, "\r\n");
                return Token(TYPES::CR, "\r");
            case '\n':  return Token(TYPES::LF, "\n");
            case ' ':   return Token(TYPES::SPACE, " ");
            case ':':   return Token(TYPES::SEMICOLON, ":");
            default:
                _letter();
                return _token;
        }
